...
async function run() {
  ...
  const assets = await sql`SELECT r2_url, category FROM assets WHERE project_id = ${projects[0].id}`;
  const svg = assets.find(a => a.category.includes('Capa Interactiva'));
  if (svg) {
    console.log('CADORE_SVG_URL=' + svg.r2_url);
  } else {
    console.log('SVG NOT FOUND IN ASSETS');
  }
}
